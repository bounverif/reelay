...
int main(int argc, char** argv)
{
  ...
  args::PositionalList<std::string> paths(
    parser,
    "FILE",
    "Log files to be checked against SPEC",
  ...
  args::Group gmodel(
    ...
    args::Options::Global);
  args::Flag fdense(
    gmodel, "fv", "Use dense time model (default)", {'v', "dense"});
  args::Flag fdiscrete(
    gmodel, "fx", "Use discrete time model", {'x', "discrete"});
  ...
  args::Group gdtype(
    ...
    args::Options::Global);
  args::Flag fint(
    gdtype, "fi", "Use int64 as time type (default)", {'i', "itime"});
  args::Flag ffloat(
    gdtype, "ff", "with -v, use float64 as time type", {'f', "ftime"});

  args::Group gsemantics(
    gsetting,
    "Value model:",
    args::Group::Validators::Xor,
    args::Options::Global);
  ...
  else if(use_dense and use_boolean and use_floating and use_linear) {  // -vbfl
    auto opts =
      reelay::dense_timed<double>::monitor<input_t, output_t>::options()
        .with_time_field_name(args::get(t_name))
        .with_value_field_name(args::get(y_name))
